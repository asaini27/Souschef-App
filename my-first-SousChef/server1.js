...
app.post('/login', (req, res) => {
  ...
  db.query('SELECT * FROM user WHERE username = ?', [username], async (err, results) => {
    ...
    if (await bcrypt.compare(password, user.password)) {
      const token = jwt.sign({ userId: user.user_id }, 'SOUS248'); // Use a proper secret key
      res.json({ token });
    } else {
...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, SOUS248, (err, user) => { // Replace 'your_secret_key' with your actual secret key
...
app.post('/ask-sous-chef', (req, res) => {
  const user_input = req.body.user_input;
  const pythonProcess = spawn('python', ['./testai2.py', user_input]);
